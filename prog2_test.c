...
void* calculate_total_sales(void* arg) {
    while (1) {
        if (processing_enabled) { //Проверяем разрешение на обработку
            update_sales_data();
            ...
            printf("Общая сумма: %.2f, Кол-во продаж: %d\n", total, sales_count);
        }
        sleep(5);
    }
    return NULL;
...
void* analyze_product_popularity(void* arg) {
    while (1) {
        if (processing_enabled) {
            ...
            printf("Анализ популярности завершен\n");
        }
        sleep(7);
    }
    return NULL;
...
void* find_top_customers(void* arg) {
    while (1) {
        if (processing_enabled) {
            ...
            printf("Анализ покупателей с определенной суммой завершен (порог: %.2f)\n", min_customer_sum);
        }
        sleep(9);
    }
    return NULL;
...
int main() {
    ...
    printf("Чтение продаж из файла: sales.txt (с названиями)\n");
